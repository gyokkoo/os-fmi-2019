...
`cut -d : -f 1,5 /etc/passwd`  или<br/>`cat /etc/passwd2 | awk -F : '{ print $1, $5 }'`
...
`find / -maxdepth 3 -type d -ls | tr -s [:blank:] | cut -c 2- | grep " d[r]" | cut -d ' ' -f 11` - Директориите до които имам достъп <br/>
`find / -maxdepth 3 -type d -ls | tr -s [:blank:] | cut -c 2- | grep " d[^r]" | wc -l` - Брой на директроиите до които нямам достъп <br/>
...
`awk "NR >= 28 && NR <= 46" /etc/passwd | cut -d : -f 3 | egrep -o '.{1}$'`
